...
/**
 * KarmiaDatabaseAdapterMemoryTableSuite
 *
 * @class
 */
class KarmiaDatabaseAdapterMemoryTableSuite {
    /**
     * Constructor
     *
     ...
     * @param {Array} tables
     * @param {*} id
     * @constructs KarmiaDatabaseAdapterMemoryTableSuite
     */
    ...
    deleter(table, id) {
        return function () {
            const parameters = [id].concat(Array.from(arguments)),
                conditions = table.key.reduce(function (collection, key, index) {
                    collection[key] = parameters[index];
...
module.exports = function (adapter, name, tables, id) {
    return new KarmiaDatabaseAdapterMemoryTableSuite(adapter, name, tables, id);
};
