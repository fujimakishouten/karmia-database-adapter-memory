...
class KarmiaDatabaseAdapterMemoryTable {
    ...
    toObject(models) {
        ...
        const self = this,
            multi = Array.isArray(models),
            result = (Array.isArray(models) ? models : [models]).map(function (model) {
                return self.fields.reduce(function (collection, key) {
                    if (key in model) {
                        collection[key] = model[key];
                    }
